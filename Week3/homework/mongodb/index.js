...
async function findEpisodesExercises(client) {
  ...
  console.log(`The title of episode 2 in season 2 is ${resultA.title}`);
  ...
  console.log(
    `The season and episode number of the "BLACK RIVER" episode is ${resultB.episode}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]

  const cursorC = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .find({ elements: "CLIFF" });

  const resultCArray = await cursorC.toArray();
  const resultCTitles = resultCArray.map((episode) => episode.title);

  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${`[ ${resultCTitles}]`}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]

  const cursorD = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .find({ $and: [{ elements: "CLIFF" }, { elements: "LIGHTHOUSE" }] });

  const resultDArray = await cursorD.toArray();
  const resultDTitles = resultDArray.map((episode) => episode.title);

  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${`[ ${resultDTitles}]`}`
  );
}
...
async function updateEpisodeExercises(client) {
  ...
  const updateResultA = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .updateOne({ episode: "S30E13" }, { $set: { title: "BLUE RIDGE FALLS" } });

  console.log(
    `Ran a command to update episode 13 in season 30 and it updated ${`(${updateResultA.modifiedCount})`} episodes`
  );
  ...
  const updateResultB = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .updateMany(
      { elements: "BUSHES" },
  ...
  console.log(
    `Ran a command to update all the BUSHES to BUSH and it updated ${`( ${updateResultB.modifiedCount} )`} episodes`
  );
}
